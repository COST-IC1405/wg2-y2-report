...
In this document we report the research activity that the working group 2 on software and system has actively undertaken during the second year of the Cost Action IC1405. There has been an evident effort from WG members to integrate reversibility with \textit{behavioural types} (e.g., session types), with \textit{recovery mechanism} and \textit{mainstream languages} such as Earlang and the \textit{obejct-oriented} paradigm.
